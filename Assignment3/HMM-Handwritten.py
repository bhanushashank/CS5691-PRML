...
def ReadInput(subdir):
    #data = {letter : [] for letter in datasets}
    data = [[] for i in range(5)]
    for letter in range(5):
        directory = 'Handwriting_Data\\' + datasets[letter] + '\\' + subdir
        for filename in os.scandir(directory):
            with open(filename.path) as f:
                for line in f:
                    ...
                    NC = values[0]
                    l = len(values)
                    points = [[float(values[i]), float(values[i+1])] for i in range(1, l, 2)]
                    data[letter].append(points)
    return data
...
def Normalization(data):
    ...
    for i in range(classes):
        ...
        for j in range(person):
            ...
            x1.sort()
            x2.sort()
            diff = [(x1[len(x1)-1]-x1[0]),(x2[len(x2)-1]-x2[0])]
            mean = [(x1[len(x1)-1]+x1[0])/2,(x2[len(x2)-1]+x2[0])/2]
            for k in range(len(data[i][j])):
                for m in range(2):
                    data[i][j][k][m] -= mean[m]
                    data[i][j][k][m] /= diff[m]
...
dev_data = Normalization(dev_data)
