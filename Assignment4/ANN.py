...
# ## Functions for ROC, DET and Confusion Matrix
...
clf = MLPClassifier(random_state=10, max_iter=1000).fit(train_data, train_data_class)
predictions = clf.predict(dev_data)
likelihoods = clf.predict_proba(dev_data)

likelihoods_image.append(likelihoods)
predictions_image.append(predictions)

accuracy = ComputeAccuracy(predictions, dev_data_class)
print(f"Accuracy is : {accuracy} %")


# TPR_k, FPR_k, FNR_k = PlotROC([likelihoods], dev_data_class, 5, [1], "ROC Curve for Image Dataset (ANN)", [""])
# PlotDET(TPR_k, FPR_k, FNR_k, "DET Curve for Image Dataset (ANN)", [""])
# ConfusionMatrix(predictions, dev_data_class, 5, "Image Dataset (ANN)")
...
TPR_k, FPR_k, FNR_k = PlotROC(likelihoods_image, dev_data_class, 5, [1, 1, 1, 1], "ROC Curve for Image Dataset (ANN)", ["Original Dataset", "Reduced dataset (PCA)", "LDA on Original Dataset", "LDA on PCA-Reduced Dataset"])
PlotDET(TPR_k, FPR_k, FNR_k, "DET Curve for Image Dataset (ANN)", ["Original Dataset", "Reduced dataset (PCA)", "LDA on Original Dataset", "LDA on PCA-Reduced Dataset"])
ConfusionMatrix(predictions_image[0], dev_data_class, 5, "Image Dataset using ANN")
...
for i in range(len(x1_train)):
    train_data.append([x1_train[i], x2_train[i]])
for i in range(len(x1_dev)):
    dev_data.append([x1_dev[i], x2_dev[i]])
...
clf = MLPClassifier(random_state=3, max_iter=1000).fit(train_data, y_train)
predictions = clf.predict(dev_data)
likelihoods = clf.predict_proba(dev_data)

likelihoods_synthetic.append(likelihoods)
predictions_synthetic.append(predictions)

accuracy = ComputeAccuracy(predictions, y_dev)
print(f"Accuracy is : {accuracy} %")
...
train_data, dev_data = ReduceDimensions_PCA(train_data, dev_data, 1)
clf = MLPClassifier(random_state=3, max_iter=1000).fit(train_data, y_train)
predictions = clf.predict(dev_data)
likelihoods = clf.predict_proba(dev_data)
accuracy = ComputeAccuracy(predictions, y_dev)
...
# ## Synthetic Dataset with LDA (original dataset)

# In[17]:


train_data, dev_data = train_data_original, dev_data_original
train_data, dev_data = ReduceDimensions_LDA(train_data, y_train, dev_data, 1)
clf = MLPClassifier(random_state=3, max_iter=1000).fit(train_data, y_train)
predictions = clf.predict(dev_data)
likelihoods = clf.predict_proba(dev_data)
accuracy = ComputeAccuracy(predictions, y_dev)
...
# ## Synthetic Dataset with LDA (PCA-reduced dataset)
...
TPR_k, FPR_k, FNR_k = PlotROC(likelihoods_synthetic, y_dev, 2, [1, 1, 1], "ROC Curve for Synthetic Dataset (ANN)", ["Original Dataset", "Reduced Dataset (PCA)", "LDA on Original Dataset"])
PlotDET(TPR_k, FPR_k, FNR_k, "DET Curve for Synthetic Dataset (ANN)", ["Original Dataset", "Reduced Dataset (PCA)", "LDA on Original Dataset"])
ConfusionMatrix(predictions_synthetic[0], y_dev, 2, "Synthetic Dataset using ANN")
...
likelihoods_handwriting.append(likelihoods)
predictions_handwriting.append(predictions)

accuracy = ComputeAccuracy(predictions, dev_data_class)
print(f"Accuracy is : {accuracy} %")


# ## Handwriting Dataset with PCA
...
TPR_k, FPR_k, FNR_k = PlotROC(likelihoods_handwriting, dev_data_class, 5, [1, 1, 1, 1], "ROC Curve for Handwriting Dataset (ANN)", ["Original Dataset", "Reduced dataset (PCA)", "LDA on Original Dataset", "LDA on PCA-Reduced Dataset"])
PlotDET(TPR_k, FPR_k, FNR_k, "DET Curve for Handwriting Dataset (ANN)", ["Original Dataset", "Reduced dataset (PCA)", "LDA on Original Dataset", "LDA on PCA-Reduced Dataset"])
ConfusionMatrix(predictions_handwriting[0], dev_data_class, 5, "Handwriting Dataset using ANN")
...
likelihoods_spoken.append(likelihoods)
predictions_spoken.append(predictions)
...
TPR_k, FPR_k, FNR_k = PlotROC(likelihoods_spoken, dev_data_class, 5, [1, 1, 1, 1], "ROC Curve for Spoken-Digit Dataset (ANN)", ["Original Dataset", "Reduced dataset (PCA)", "LDA on Original Dataset", "LDA on PCA-Reduced Dataset"])
PlotDET(TPR_k, FPR_k, FNR_k, "DET Curve for Spoken-Digit Dataset (ANN)", ["Original Dataset", "Reduced dataset (PCA)", "LDA on Original Dataset", "LDA on PCA-Reduced Dataset"])
ConfusionMatrix(predictions_spoken[0], dev_data_class, 5, "Spoken-Digit Dataset using ANN")
